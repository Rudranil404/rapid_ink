...
@section('content')
    <style>
        ...
        .table thead th { background-color: #f9fafb; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280; padding: 16px 24px; border-bottom: 1px solid var(--border-color); white-space: nowrap; }
        .table tbody td { padding: 16px 24px; vertical-align: middle; font-size: 14px; font-weight: 500; border-bottom: 1px solid var(--border-color); white-space: nowrap; }
        ...
        @media (max-width: 768px) {
            .header-actions { flex-direction: column; align-items: stretch !important; gap: 12px; }
            .header-actions h5 { display: none; /* Hide title to save space, it's in topbar */ }
        }
    </style>

    <div class="d-flex justify-content-between align-items-center mb-4 header-actions">
        <h5 class="fw-bold mb-0">All Products</h5>
        <a href="{{ route('admin.products.create') }}" class="btn btn-brand d-flex align-items-center justify-content-center gap-2">
                            ...
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center gap-3" style="min-width: 250px;">
                                        ...
                                        @endif
                                        <div>
                                            <div class="fw-bold text-dark text-wrap" style="line-height: 1.2;">{{ $product->name }}</div>
                                            <div class="text-muted small mt-1">ID: #PRD-{{ str_pad($product->id, 4, '0', STR_PAD_LEFT) }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="text-end" style="min-width: 140px;">
                                    <form action="{{ route('admin.products.toggle-trending', $product->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm border me-1 {{ $product->is_trending ? 'bg-warning text-dark border-warning' : 'btn-light text-muted' }}" title="Toggle Trending">
